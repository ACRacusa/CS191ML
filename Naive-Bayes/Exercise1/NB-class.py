...
nb=NaiveBayes(np.unique(label_train)) #instantiate a NB class object

print ("---------------- Training In Progress --------------------")

nb.train(msg_train,label_train) #start training by calling the train function

print ('----------------- Training Completed ---------------------')

pclasses=nb.test(msg_test) #get predcitions for test set

#check how many predcitions actually match original test labels
test_acc=np.sum(pclasses==label_test)/float(label_test.shape[0]) 

print ("Test Set Examples: ",label_test.shape[0])
